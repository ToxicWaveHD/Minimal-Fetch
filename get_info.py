...
def verify(package):
  found = True
  try: get(str("whereis " + package)).split(": ")[1]
  except: found = False
...
def find_gpu():
  ...
  fancy_gpu = gpu.replace("Intel Corporation ", "").replace(" Integrated Graphics Controller", "").replace("Core Processor ", "")
...
def find_memory():
  ...
  total_mem = int(int(mem.split()[1]) / 100)#  The double int is used to serve as a truncate
  used_mem = int(int(mem.split()[2]) / 100)#   The double int is used to serve as a truncate
...
try: sysinfo["wm"] = find_wm()
except: sysinfo["wm"] = "null"

try: sysinfo["os"] = find_os()
except: sysinfo["os"] = "null"

try: sysinfo["cpu"] = find_cpu()
except: sysinfo["cpu"] = "null"

try: sysinfo["memory"] = find_memory()
except: sysinfo["memory"] = "null"

try: sysinfo["kernel"] = find_kern()
except: sysinfo["kernel"] = "null"

try: sysinfo["packages"] = find_packages()
except: sysinfo["packages"] = "null"

try: sysinfo["gpu"] = find_gpu()
except: sysinfo["gpu"] = "null"
